...
public class UtentiControllerTest
{
	...
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/userid/Umberto")
				.accept(MediaType.APPLICATION_JSON))
				...
				.andExpect(jsonPath("$.userId").value("Umberto"))
				...
				assertThat(passwordEncoder.matches("password", 
						utentiRepository.findByUserId("Umberto").getPassword()))
				.isEqualTo(true);
	}
	...
	public void D_testGetAllUser() throws Exception
	{
		mockMvc.perform(MockMvcRequestBuilders.get("/api/utenti/cerca/tutti")
				...
				.andExpect(jsonPath("$[0].userId").value("Umberto"))
	...
	{
		mockMvc.perform(MockMvcRequestBuilders.delete("/api/utenti/elimina/Umberto")
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.code").value("200 OK"))
				.andExpect(jsonPath("$.message").value("Eliminazione Utente Umberto Eseguita Con Successo"))
				.andDo(print());
	}
